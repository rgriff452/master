...
namespace NewTake.model
{
    public class World
    {
       ...
       // public const byte VIEW_DISTANCE_NEAR_Z = VIEW_CHUNKS_Z * 2;
        ...
        public void VisitChunksAtPosition(Func<Vector3i,Chunk> visitor,Vector3i position,byte radius)
        {
            for (uint x = position.X - radius; x < position.X + radius + 1; x++)
            {
                for (uint z = position.Z - radius; z < position.Z + radius + 1; z++)
                {
                    visitor(new Vector3i(x, 0, z));
                }
            }
