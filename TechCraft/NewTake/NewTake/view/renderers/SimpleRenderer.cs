...
namespace NewTake.view.renderers
{
    class SimpleRenderer : IRenderer
    {
        ...
        {
            _world.VisitChunksAtPosition(DoGenerateForUpdate, new Vector3i(_player.position), 6);
        }
        ...
        private Chunk DoGenerateForUpdate(Vector3i chunkIndex)
        {
            Chunk chunk = _world.Chunks[chunkIndex.X, chunkIndex.Z];
            if (chunk == null)
            {
                chunk = DoGenerate(chunkIndex);
                return DoGenerate(chunk);
            }
            else
            {
